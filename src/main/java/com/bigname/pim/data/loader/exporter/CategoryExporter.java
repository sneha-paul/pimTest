...
public class CategoryExporter {
    ...
    public boolean exportData(String filepath){
        ...
        data.put("1", new Object[]{"CATEGORY_ID", "CATEGORY_NAME","DESCRIPTION","ACTIVE", "DISCONTINUED" });

        int i=2;
        for (Iterator<Category> iter = categoryData.iterator(); iter.hasNext(); ) {
            Category element = iter.next();
            data.put(Integer.toString(i), new Object[]{element.getCategoryId(), element.getCategoryName(),element.getDescription(), element.getActive(),element.getDiscontinued() });
            i++;
        }
