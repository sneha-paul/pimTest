            ...
            var config = $.isPlainObject(options) ? options : {},
                ...
                conditionalPaging = function(e) {
                    var $paging = $(api.table().container()).find('div.dataTables_paginate'),
                        $pageSize = $(api.table().container()).find('div.dataTables_length'),
                    ...
                    if (e instanceof $.Event) {
                        if (pages <= 1) {
                            if (config.style === 'fade') {
                                ...
                                $pageSize.stop().fadeTo(speed, 0);
                            }
                            else {
                                $paging.css('visibility', 'hidden');
                                $pageSize.css('visibility', 'hidden');
                            }
                        }
                        else {
                            if (config.style === 'fade') {
                                $paging.stop().fadeTo(speed, 1);
                                $pagSize.stop().fadeTo(speed, 1);
                            }
                            else {
                                $paging.css('visibility', '');
                                $pageSize.css('visibility', '');
                            }
                        }
                    }
                    else if (pages <= 1) {
